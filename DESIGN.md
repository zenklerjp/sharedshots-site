...
- **Printed table card:** Fresh Print on a square-cornered plate with an ink hairline, a Paper Plate shadow and a small rotation, standing in for a card a host prints and puts on a table. It carries the brand mark, one line of guest instruction, a QR, and the event code at display weight with wide tracking. The QR is **generated from a URL, not drawn** - error level Q, a four-module quiet zone inside the viewBox, `shape-rendering="crispEdges"` so module edges are not antialiased into grey, and ink on Fresh Print, never inverted. A decorative QR that does not scan is the one thing this card must never be, because it sits in the section telling hosts to print QR codes. The card's own QR resolves to the site; the caption says so plainly, since the one a host prints resolves to their album instead. The card's instruction must stay true to what the site can do: it reads "enter the code at sharedshots.site" only because `album.html` now accepts a typed code.
